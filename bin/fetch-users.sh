...
EXPORT_JOB_ID=$(curl -L 'https://dust-dev.eu.auth0.com/api/v2/jobs/users-exports' \
...
while true; do
  STATUS=$(curl -L "https://dust-dev.eu.auth0.com/api/v2/jobs/$EXPORT_JOB_ID" \
  -H 'Accept: application/json' \
  -H "Authorization: Bearer ${AUTH0_API_TOKEN}" \
  -s | jq -r .status)
...
URL=$(curl -L "https://dust-dev.eu.auth0.com/api/v2/jobs/$EXPORT_JOB_ID" \
  -H 'Accept: application/json' \
  -H "Authorization: Bearer ${AUTH0_API_TOKEN}" \
  -s | jq -r .location)
